...
export const ModalFacturaDeuda = ({ showModalFacturaDeuda, setShowModalFacturaDeuda, datosFacturaDeuda }) => {

    const cerrarModal = () => {
        setShowModalFacturaDeuda(false)
    }

    const { codigoFacturaDeuda, datosDeuda, productosDeuda, historialPagos } = datosFacturaDeuda
            ...
            >
                ...
                >
                    <div className="w-full">
                        ...
                        <Heading fontSize={'larger'} textAlign={'center'}>VENTA A CREDITO</Heading>
                        ...
                        <div className="w-full">
                            ...
                            <Text>
                                Estado: {estado ? 'Pendiente' : 'Saldado'}
                            </Text>
                            <Text>
                                Fecha: {fecha}
                            </Text>
                    ...
                    <div className="w-full mt-[5px] flex flex-col">
                        <Heading fontSize={'larger'} textAlign={'center'} marginBottom={'10px'}>Historial De Pagos</Heading>

                        {historialPagos.length == 0 ? <Text fontSize={'large'} textAlign={'center'}>No hay pagos realizados</Text> : (
                                ...
                                return (
                                    <div className="flex gap-3" key={e.id}>
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <Text fontSize={'large'}>{formatoDinero(e.motoPago)}</Text>
                                        </div>
                                    </div>
                                )
                            })
                    ...
                    <div className=" w-full flex flex-wrap gap-[8px] mt-[5px]">
                        ...
                        <div>
                            ...
                            <Text fontSize={'large'}>{formatoDinero(parseFloat(montoInicial))}</Text>
                        </div>

                        <div>
                            <Heading fontSize={'larger'} >Monto Actual:</Heading>
                            <Text fontSize={'large'}>{formatoDinero(parseFloat(montoActual))}</Text>
                        </div>
                ...
                <ModalFooter display={'flex'} flexDirection={'column'} gap={2}>
                    <PDFDownloadLink document={<PlantillaFacturaDeuda datos={datosFacturaDeuda} />} fileName={`facturaCredito - ${codigoFacturaDeuda}.pdf`}>
