...
export const buscarProductos = async (req, res) => {
    ...
    try {
        ...
        if (existeProducto) {
            ...
            res.status(404).json({ msg: 'No existe este producto en la base de datos' })
        }
    } catch (error) {
        console.log(error);
    }
...
export const buscarPorNombre = async (req, res) => {
    ...
    try {
        ...
        if (existeProducto.length == 0) {
            return res.status(404).json({ msg: 'No existen productos con ese nombre' })
        }
...
export const agregarProductos = async (req, res) => {
    ...
    const { codigoProducto, nombre, cantidadStock, minStock, precioCompraUnd, ganancia } = req.body;
...
export const eliminarProductos = async (req, res) => {
    ...
    try {
        ...
        const productosEliminados = await prisma.$queryRaw`DELETE FROM historialpagosdeudas`;
...
export const actualizarProductos = async (req, res) => {
    const { codigoProducto } = req.params;
    const { nombreNuevo, cantidadStockNuevo, minStockNuevo, precioCompraUndNuevo, gananciaNuevo } = req.body;
...
export const venderProductos = async (req, res) => {
    ...
    try {
        const respuesta = await prisma.$transaction(
            productosVendidos.map((e) => {
                return prisma.productos.update({
                    ...
                    select: {
                        codigoProducto: true, nombre: true, cantidadStock: true, minStock: true, enBaja: true, agotado: true
                    }
                })
            })
        )
        ...
        if (existeProductoEnBaja) {
            ...
            const productosEnBaja = respuesta.filter(e => e.cantidadStock <= e.minStock);
