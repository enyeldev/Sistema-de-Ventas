...
export const agregarDeuda = async (req, res) => {
    ...
    try {
        ...
        const { nombreCliente, telefonoCliente, fecha, montoDeuda, fechaUltimoPago, usuarioCajeroId, despachadoPor, montoActualDeuda } = req.body;
...
export const agregarProductosEnDedua = (req, res) => {
    ...
    try {
        productosDeudas.forEach(async (e) => {
            const {
                codigoDeuda, codigoProducto, cantidad, precioVentaUnd, total, nombre
            } = e
...
export const buscarDeudaPorCodigo = async (req, res) => {
    ...
    try {
        ...
        if (!existeFactura) {
            ...
            return res.status(404).json({ msg: 'El codigo de la factura es invalido' })
        }
...
export const buscarCodigoFactura = async (req, res) => {
    ...
    try {
        ...
        res.json({ msg: 'Codigo facuta encontrado', codigo: factura.codigoFacturaDeuda })
    } catch (error) {
        console.log(error);
        res.status(400).json({ msg: error })
    }
